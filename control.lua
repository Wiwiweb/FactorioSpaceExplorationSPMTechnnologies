...
local function find_infinite_mining_prod_tech()
  local infinite_techs = prototypes.get_technology_filtered{{filter="max-level", comparison=">", value=9999999}}
  for name, _tech in pairs(infinite_techs) do
    if string.starts(name, "mining-productivity") then
      log("Infinite mining tech is: " .. name)
      storage.infinite_mining_tech_name = name
      return
    end
  end
  storage.infinite_mining_tech_name = nil
end
...
local function on_research_finished(event)
  ...
  if string.starts(technology.name, "se-spm-bragging-rights") then
    ...
    elseif storage.infinite_mining_tech_name then
      local infinite_mining_tech = force.technologies[storage.infinite_mining_tech_name]
